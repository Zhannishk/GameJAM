...
class Player:
    def __init__(self, x, y):
        ...
        self.y = int(y)
        self.player_size = 40
        self.player = pygame.image.load("player/left_leg_b.png")
    ...
    def got_stuff(self, tile, grid_cells, screen):
        ...
        stuff = None
        if current_cell.stuff:
            print(f"has stuff: {current_cell.stuff}")
            stuff = current_cell.stuff
            current_cell.set_stuff(None)
            current_cell.draw(screen, tile)
        return stuff

    # stops player to pass through walls
    def check_move(self, tile, grid_cells, thickness, screen):
        current_cell_x, current_cell_y = self.x // tile, self.y // tile
        current_cell = self.get_current_cell(current_cell_x, current_cell_y, grid_cells)
